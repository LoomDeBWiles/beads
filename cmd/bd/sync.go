...
func exportToJSONL(ctx context.Context, jsonlPath string) error {
	...
	// calls back for the content, and records the hash across the rename, so no
	// reader can see the new file before the metadata that describes it.
	if _, err := jsonlpub.Publish(ctx, store, jsonlPath, func(ctx context.Context) ([]*types.Issue, error) {
		return collectSyncExportIssues(ctx, jsonlPath)
...
// collectSyncExportIssues gathers the full contents of the JSONL mirror. It runs
// inside the publish lock, so the issues it reads are the issues that get written.
func collectSyncExportIssues(ctx context.Context, jsonlPath string) ([]*types.Issue, error) {
	...
	// Sorting by ID is the publisher's job - it sorts as it writes.
