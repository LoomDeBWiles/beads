...
| `internal/storage/sqlite/multirepo.go` | Add the column to the INSERT copy path (:302-305) | Multi-repo copy otherwise writes rows without leases |
...
| `internal/importer/utils.go` | Add `case "claim_expires_at"` to `checkFieldChanged` (:124-149; `default:` returns false) | Without it a lease-only change reads as "unchanged" and `UpdateIssue` is never called, so a renewal never propagates |
...
| Renewal-only change invisible to import | `content_hash` excludes the lease (types.go:67-88), so the importer's exact-hash short-circuit (importer.go:602-606) skips lease-only renewals; cross-clone lease views lag until the next status/assignee change. Acceptable: single-host fleet, claims run against the primary store |
