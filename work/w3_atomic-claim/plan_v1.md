...
```
bd claim <id> --assignee X [--lease 30m] [--json]
  │
...
Exit codes: `0` claimed/renewed/stolen, `3` denied (stderr names holder and expiry; `--json` emits the outcome object), `1` error (not found, closed, invalid args, DB failure). Exit 3 lets shell callers branch on "lost the race" without parsing JSON.
...
JSONL: `ClaimExpiresAt *time.Time` with `json:"claim_expires_at,omitempty"` on `types.Issue` round-trips through export/import automatically. The git merge driver does NOT: `internal/merge/merge.go`'s `Issue` struct models 14 fields and `mergeIssue` rebuilds conflicting rows from it, silently dropping unmodeled fields — `assignee` is already dropped on true conflicts today (pre-existing defect, same root). Fix both: model `assignee` and `claim_expires_at` in `merge.Issue`/`mergeIssue`, merged as a pair by latest `updated_at` (they change together in a claim), cleared when the merged status is not `in_progress`.
...
| `internal/storage/sqlite/queries.go` | `manageClaimExpiry`: status leaving `in_progress` appends `claim_expires_at = NULL` (mirror `manageClosedAt`); wire into `UpdateIssue` and `CloseIssue`; add column to `scanIssueRow`-equivalent scanners and explicit SELECT/INSERT column lists (enumerate by `pinned` grep) | Lease lifecycle; field visible everywhere rows are read |
...
| `internal/storage/sqlite/claim_test.go` | New: outcome matrix (open→claimed; held-unexpired→denied+no write; self→renewed; expired→stolen+event has prior holder; closed→error; no-lease claim never expires); race test: N=8 goroutines claim one open issue, assert exactly 1 outcome=claimed, 7 denied, final assignee = winner | Proves the CAS |
...
| `cmd/bd/claim.go` | New cobra command `claim <id>` (GroupID issues): `--assignee` (default `$BEADS_ACTOR`/git user like other actor resolution — reuse existing actor helper), `--lease` duration, `--json`; `CheckReadonly("claim")`; partial-ID resolve like update; route daemon/direct; exit map per Design | The verb |
...
### Phase 3: JSONL round-trip + merge driver — Gate: `go test ./internal/merge/... ./cmd/bd/ -run 'Export|Import|Merge'` passes

| File | Change | Why |
|------|--------|-----|
| `cmd/bd/export_test.go` (or nearest round-trip test) | Round-trip test: claimed issue with lease → export → import → `claim_expires_at` identical | Prove the json tag actually survives both paths (import may rebuild via explicit fields) |
...
| `internal/merge/merge_test.go` | Conflict cases: both sides claim (latest updated_at wins, one holder survives); one side closes + one claims (closed wins per `mergeStatus`, lease cleared) | Merge semantics locked by test |
...
| `internal/importer/importer.go` | Yes* | Round-trip proven by Phase 3 test; only touched if that test exposes an explicit-field rebuild gap (then: add field mapping, nothing else) |
...
6. Round-trip: export → import into fresh store → lease field identical.
