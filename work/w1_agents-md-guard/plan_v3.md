...
| Finding | Disposition | Change |
|---------|-------------|--------|
| F1 | ACCEPT | Expanded the factory no-touch regression test to cover every former install and remove mutation path. |
| F1b | ACCEPT | Specified five factory test cases and byte-level assertions in the Phase 1 file table. |
| F2 | ACCEPT | Replaced template heuristics with two exact measured hashes and corrected the executor’s deletion command. |
| F3 | ACCEPT | Corrected the factory function and test names and added the command help text to Phase 1 scope. |
| F3b | ACCEPT | Updated the Phase 1 implementation rows and gate to use the real factory names and test suite. |
| F4 | ACCEPT | Made binary backups write-once with a verified manifest and extended the Phase 2 gate through check 5. |
| F5 | ACCEPT | Strengthened binary identity checks and isolated the factory E2E test in its own scratch repository. |
...
- `InstallFactory` — replace all three write branches (create/append/update, lines ~105-152) with printing the template block and instructions to stdout, no filesystem writes.
...
- `cmd/bd/setup.go` — update the factory command's long help (lines 75-78: 'Creates or updates AGENTS.md...') and the `--remove` flag description (line 122: 'Remove bd section from AGENTS.md') to say the command PRINTS instructions for manual application and never writes AGENTS.md.
...
### Phase 1: Remove the feature from the fork — Gate: `go build ./...` clean and `go test ./cmd/bd/... -run 'TestInit|TestFactory' -v` passes, including the new no-touch tests
...
| `cmd/bd/setup/factory.go` | InstallFactory prints the integration block instead of writing AGENTS.md; RemoveFactory prints removal instructions instead of editing the file | close the second AGENTS.md writer |
| `cmd/bd/setup.go` | factory command long help + --remove flag description rewritten: command prints instructions, never writes AGENTS.md | help must not falsely claim file mutation |
...
| `cmd/bd/setup/factory_test.go` (new file or extend existing setup tests) | `TestFactoryDoesNotTouchAgentsFile` — five subtests: install × {absent, plain, marked} and --remove × {marked-only, curated-marked}, asserting absent or byte-identical after every command | pins the no-write behavior |
...
### Phase 2: Reinstall both binary copies + E2E — Gate: proof checks 1-5 below pass with pasted output

| Step | Command | Why |
|------|---------|-----|
| Backup current binaries | `test -e ~/projects/tools/beads/work/w1_agents-md-guard/backup/MANIFEST.sha256 || { mkdir -p ~/projects/tools/beads/work/w1_agents-md-guard/backup && cp ~/.local/bin/bd ~/projects/tools/beads/work/w1_agents-md-guard/backup/bd.local-bin && cp ~/go/bin/bd ~/projects/tools/beads/work/w1_agents-md-guard/backup/bd.go-bin && (cd ~/projects/tools/beads/work/w1_agents-md-guard/backup && sha256sum bd.local-bin bd.go-bin > MANIFEST.sha256); }; (cd ~/projects/tools/beads/work/w1_agents-md-guard/backup && sha256sum -c MANIFEST.sha256)` | Rollback without rebuild; persistent primary path that exists after the worktree is merged and removed; backups stay untracked (never committed — 33 MB each); hashes prove the copies are intact; the MANIFEST guard makes the backup write-once — a resume after a partial install re-verifies the original copies instead of overwriting them with already-installed binaries |
...
Pre-deletion gate (per file, at execution time, inside the fresh worktree): `sha256sum` of the file must equal one of the two measured generated-template hashes — `bf83e48f58204bea5bee22b987a833b36deedd15253ddafd6bac106a78c1ded2` (the 40-line variant, 12 repos) or `92c1bfa374c21a0f141a06fe8516423d713596c4220f13b7da35eb35a2bb335c` (spec-code's 39-line variant). Any other hash → do not delete; stop and report that repo for manual review (a concurrent curated edit must never be trashed).
...
Executor: the manager runs this phase directly. Rationale (deviation from "manager dispatches"): each repo change is one file deletion or one section strip, and the surrounding `wt-new`/`wt-merge` lifecycle is owner-only tooling that workers are forbidden to run; dispatching 14 builders for one-line edits inside manager-created worktrees adds cost and risk without review value. The edits are made per-file with the Edit tool / `trash.sh` + `git add -u`, not a bulk script.
...
**Tests:** `cd ~/worktrees/beads/w1_agents-md-guard && go build ./... && go test ./cmd/bd/... -run 'TestInit|TestFactory' -v`.
...
|---|-------|---------|----------|
| 1 | Unit + regression tests | `go test ./cmd/bd/... -run 'TestInit|TestFactory' -v` | all PASS incl. TestInitDoesNotTouchAgentsFile + TestFactoryDoesNotTouchAgentsFile |
...
| 4 | PATH binary is the fixed build | `command -v bd && bd version && sha256sum ~/.local/bin/bd ~/go/bin/bd && git -C ~/projects/tools/beads rev-parse HEAD` | `/home/ben/.local/bin/bd`; the two binary hashes are EQUAL; the commit embedded in `bd version` is a prefix of the printed merged-main HEAD |
| 5 | Factory setup does not write (run inside `$d2`) | `d3=$(mktemp -d) && cd $d3 && git init -q && bd init -q && bd setup factory && test ! -e AGENTS.md && echo NOWRITE` | prints the integration block, then `NOWRITE` (own temp repo; independent of checks 2-3) |
