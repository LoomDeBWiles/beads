...
```
1. acquire publish lock (internal/lockfile, .beads/.publish.lock; ctx-aware — R3-8)
2. guard: ContentState of the canonical file, under this lock              ── R3-2
   └ diverged → ABORT (import first) — validatePreExport's semantic,
     now atomic with the rename it protects: pulled-but-unimported
     bytes can never be overwritten
...
6. os.Rename(temp, issues.jsonl) + chmod
7. SetMetadata(jsonl_content_hash, H_new) + SetJSONLFileHash
   + last_import_time(RFC3339Nano) + delete pending                        ── promote (best-effort)
8. conditional dirty clear: DELETE WHERE issue_id=? AND marked_at=?        ── loop dies (R6)
9. release lock
```
...
file hash == pending   → fresh          (between 6 and 7, or crash there); a LOCKED reader
...
file old + pending set → fresh          (between 5 and 6, or crash there: old hash == committed)
...
State walk: crash after 5 → file = committed, fresh; `validatePreExport` does not refuse (hasJSONLChanged accepts committed-or-pending — v1's single-key version deadlocked exactly here); next publish overwrites pending. Crash after 6 → file = pending, fresh; the first locked event (reader promotion, publish, or RecordImport) promotes/clears. Promotion failure at 7 → warn; pending keeps readers correct. Mid-export mutation → upsert refreshes `marked_at` (`dirty_helpers.go:14-18`), step 8's conditional delete leaves the row dirty, next flush exports it. Accepted residual (R3-4, see Risks): between a crashed post-rename publish and the first locked event, a manual restore of the exact old committed bytes reads fresh; it self-corrects at the next publish or import.
...
| `cmd/bd/import.go` | Post-import metadata block (:388-396) replaced by `RecordImport` with the parsed-bytes hash — **only when the import source is the canonical default JSONL** (R3-5); `-i backup.jsonl` imports leave canonical metadata untouched | V2-4 + R3-5 |
...
LOG=$(ls .beads/daemon-*.log 2>/dev/null | tail -1 || echo .beads/daemon.log)
B0=$(grep -c "Flushing" "$LOG" 2>/dev/null || echo 0)
cp .beads/issues.jsonl /tmp/e2e_same.$$ && mv /tmp/e2e_same.$$ .beads/issues.jsonl
for i in $(seq 1 20); do F=$(grep -c "Flushing" "$LOG" 2>/dev/null || echo 0); [ "$F" -gt "$B0" ] && break; sleep 0.5; done
D=$(sqlite3 .beads/beads.db "SELECT COUNT(*) FROM dirty_issues"); echo "flushes=$((F-B0)) dirty_after_flush=$D"
test "$((F-B0))" = 1; test "$D" = 0
cp .beads/issues.jsonl /tmp/e2e_same.$$ && mv /tmp/e2e_same.$$ .beads/issues.jsonl
sleep 10
F2=$(grep -c "Flushing" "$LOG" 2>/dev/null || echo 0); echo "flushes_after_second_touch=$((F2-B0))"
...
"$BD" daemon --stop
echo "SCRATCH_DIR=$S (leave for trash.sh)"
echo E2E_PASS
```

Bounded polling replaces every fixed wait except the 10-second negative check (which must observe absence, not presence). The builder verifies `init --prefix t --quiet` and `create --json` against `$BD --help` before the run and corrects the plan via the normal defect route if a flag differs.
